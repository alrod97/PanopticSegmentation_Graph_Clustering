...
from common import grid_subsampling, batch_neighbors
from config_KPFCNN import bcolors
...
class SemanticKittiDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', balance_classes=True, seqential_batch = False):
        ...
        # Dataset folder
        self.path = '../4D-PLS/data/SemanticKitti'
        ...
        print(f'frames.type: {type(self.frames[0])}')
        ...
        while True:
            ...
            if n > self.max_in_p * self.gpu_r:
                ...
                if self.config.sampling == 'density':
                    ...
                    input_inds = np.random.choice(n, size=int(self.max_in_p*self.gpu_r),
                                                  replace=False, p=densities/np.sum(densities))
                else:
                    # random sampling
                ...
                # xxxsam points might get downsampled here
